...
export async function initSessionState(params: {
  ...
  // For fresh sessions (/new, /reset, idle rollover), replace the entry instead of
  // merging so stale runtime-only fields (for example cliSessionIds) are dropped.
  // For existing sessions, keep merge behavior to preserve unknown legacy keys.
  ...
  await updateSessionStore(
    storePath,
    (store) => {
      store[sessionKey] = isNewSession ? sessionEntry : { ...store[sessionKey], ...sessionEntry };
    },
    {
