...
describe("initSessionState preserves behavior overrides across /new and /reset", () => {
  ...
  it("/new clears stale CLI resume session ids", async () => {
    const storePath = await createStorePath("openclaw-reset-cli-");
    const sessionKey = "agent:main:telegram:dm:user-cli";
    const existingSessionId = "existing-session-cli";
    await seedSessionStoreWithOverrides({
      storePath,
      sessionKey,
      sessionId: existingSessionId,
      overrides: {
        verboseLevel: "on",
        cliSessionIds: { "codex-cli": "stale-codex-thread" },
        claudeCliSessionId: "stale-legacy-thread",
      },
    });
    ...
    const persistedRaw = await fs.readFile(storePath, "utf-8");
    const persistedStore = JSON.parse(persistedRaw) as Record<string, Record<string, unknown>>;
    expect(persistedStore[sessionKey]?.cliSessionIds).toBeUndefined();
    expect(persistedStore[sessionKey]?.claudeCliSessionId).toBeUndefined();
    expect(persistedStore[sessionKey]?.verboseLevel).toBe("on");
  });
