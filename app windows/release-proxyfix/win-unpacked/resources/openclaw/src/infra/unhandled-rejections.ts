...
function isFailoverError(err: unknown): boolean {
  ...
  if (typeof err === "string") {
    return err.includes("FailoverError");
  }

  if (err instanceof Error) {
    if (err.name === "FailoverError") {
      return true;
    }
    if (err.message.includes("FailoverError")) {
      return true;
    }
  } else if (typeof err === "object" && "name" in err && String(err.name) === "FailoverError") {
    return true;
  }

  const cause = getErrorCause(err);
  if (cause && cause !== err && isFailoverError(cause)) {
    return true;
  }

  if (err instanceof AggregateError && err.errors?.length) {
    return err.errors.some((entry) => isFailoverError(entry));
  }
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    // FailoverError means a model/provider run failed in a recoverable way.
    // Some async callsites may still leak this to process-level handlers;
    // do not take down the gateway for a single failed run.
    if (isFailoverError(reason)) {
      console.warn(
        "[openclaw] Non-fatal failover rejection (continuing):",
