...
describe("installUnhandledRejectionHandler - fatal detection", () => {
  ...
  describe("non-fatal errors", () => {
    it("does NOT exit on FailoverError", () => {
      const failoverErr = Object.assign(new Error("provider failover"), {
        name: "FailoverError",
      });

      process.emit("unhandledRejection", failoverErr, Promise.resolve());
