...
describe("buildAgentSystemPrompt", () => {
  ...
  it("omits heartbeat section unless a heartbeat prompt is provided", () => {
    const regular = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
    });
    expect(regular).not.toContain("## Heartbeats");
    expect(regular).not.toContain("HEARTBEAT_OK");

    const heartbeat = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      heartbeatPrompt: "ping",
    });
    expect(heartbeat).toContain("## Heartbeats");
