...
function summarizeSubagentAnnounceError(err: unknown): string {
...
function startSubagentAnnounceCleanupFlow(runId: string, entry: SubagentRunRecord): boolean {
  ...
  void runSubagentAnnounceFlow({
    ...
    .catch((err) => {
      // Never leak announce failures as unhandled rejections: that can terminate
      // the gateway and drop webchat sockets (observed as disconnected 1006).
      console.warn(
        `[openclaw] subagent announce flow failed for run ${runId}: ${summarizeSubagentAnnounceError(err)}`,
      );
