...
describe("subagent registry steer restarts", () => {
  ...
  it("does not leak unhandled rejection when announce flow throws", async () => {
    ...
    try {
      mod.registerSubagentRun({
        runId: "run-announce-error",
        childSessionKey: "agent:main:subagent:announce-error",
        requesterSessionKey: "agent:main:main",
        requesterDisplayKey: "main",
        task: "announce error",
        cleanup: "keep",
      ...
      const afterFailure = mod.listSubagentRunsForRequester("agent:main:main")[0];
      expect(afterFailure?.cleanupHandled).toBe(false);
      expect(afterFailure?.cleanupCompletedAt).toBeUndefined();
      ...
      const afterRetry = mod.listSubagentRunsForRequester("agent:main:main")[0];
      expect(typeof afterRetry?.cleanupCompletedAt).toBe("number");
    } finally {
