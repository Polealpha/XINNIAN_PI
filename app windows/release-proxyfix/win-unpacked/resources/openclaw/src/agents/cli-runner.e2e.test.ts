...
describe("runCliAgent resume cleanup", () => {
  ...
  it("kills stale resume processes for codex sessions", async () => {
    ...
    runExecMock
      ...
      .mockResolvedValueOnce({
        stdout: [
          `  ${selfPid + 1} ${selfPid} codex exec --json --color never --sandbox read-only --skip-git-repo-check resume thread-123`,
          `  ${selfPid + 2} 999 codex exec --json --color never --sandbox read-only --skip-git-repo-check resume thread-123`,
        ].join("\n"),
        stderr: "",
  ...
  it("retries once when codex exits with benign warning-only stderr", async () => {
    runExecMock.mockResolvedValue({ stdout: "", stderr: "" });
    runCommandWithTimeoutMock
      .mockResolvedValueOnce({
        stdout: "",
        stderr: [
          "2026-02-22T12:09:24.590468Z WARN codex_core::state_db: state db record_discrepancy: find_thread_path_by_id_str_in_subdir, falling_back",
          "2026-02-22T12:09:24.715709Z WARN codex_core::shell_snapshot: Failed to create shell snapshot for powershell: Shell snapshot not supported yet for PowerShell",
        ].join("\n"),
        code: 1,
        signal: null,
        killed: false,
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({ item: { type: "message", text: "recovered" } }),
        stderr: "",
        code: 0,
        signal: null,
        killed: false,
    ...
    const result = await runCliAgent({
      ...
      model: "gpt-5.3-codex",
      timeoutMs: 1_000,
      runId: "run-retry-benign-warning",
    });

    expect(runCommandWithTimeoutMock).toHaveBeenCalledTimes(2);
    expect(result.payloads?.[0]?.text).toBe("recovered");
  });

  it("accepts codex text output even when exit code is non-zero with benign warnings", async () => {
    runExecMock.mockResolvedValue({ stdout: "", stderr: "" });
    runCommandWithTimeoutMock.mockResolvedValueOnce({
      stdout: JSON.stringify({ item: { type: "message", text: "ok-from-stdout" } }),
      stderr:
        "2026-02-22T12:09:24.715709Z WARN codex_core::shell_snapshot: Failed to create shell snapshot for powershell: Shell snapshot not supported yet for PowerShell",
      code: 1,
      signal: null,
    ...
    const result = await runCliAgent({
      ...
      model: "gpt-5.3-codex",
      timeoutMs: 1_000,
      runId: "run-accept-benign-warning",
    });

    expect(runCommandWithTimeoutMock).toHaveBeenCalledTimes(1);
    expect(result.payloads?.[0]?.text).toBe("ok-from-stdout");
  });
});
