...
describe("gateway server chat", () => {
  ...
  test("chat.send avoids gateway-injected duplicate assistant messages once agent run starts", async () => {
    const tempDirs: string[] = [];
    const { server, ws } = await startServerWithClient();
    const spy = vi.mocked(getReplyFromConfig);
    ...
    try {
      ...
      spy.mockReset();
      spy.mockImplementationOnce(async (_ctx, opts) => {
        opts?.onAgentRunStart?.(opts.runId ?? "idem-no-dup-1");
      ...
      if (transcriptExists) {
        ...
        const rows = transcript
          ...
          .map((line) => JSON.parse(line) as { message?: { model?: string } });
        const injected = rows.filter((row) => row.message?.model === "gateway-injected");
